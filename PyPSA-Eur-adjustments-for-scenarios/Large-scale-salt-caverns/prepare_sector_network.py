...
def add_storage_and_grids(n, costs):
    ...
    #FNB_cavern_types = list(snakemake.params.sector["FNB_hydrogen_underground_storage_locations"].keys())
    #h2_caverns_FNB = pd.read_csv(snakemake.input.h2_cavern_FNB, index_col=0)
    h2_caverns_FNB = pd.DataFrame({
        'name': ['FNB1','FNB2','FNB3','FNB4','FNB5','FNB6','FNB7','FNB8','FNB9','FNB10','FNB11','FNB12'],
        'capacity': [1805400,382000,3640000,150000,63000,99100,100000,3000,700,None,1000,177000], # MWh
        'x': [7.031767126,7.388594254,7.309570959,7.007256,7.908380876,6.989492445,6.426959018,9.121749081,7.037931754,8.401425983,9.481373381,11.83585297], # longtitude
        'y': [51.4864889,53.25349602,52.52201096,52.17687752,53.45438063,52.17298166,51.68047009,54.1955015,53.41659566,53.19500133,53.43655422,51.4278057], # latitude
        #'PyPSA_bus': []
    })
    h2_caverns_FNB['Coordinates'] = list(zip(h2_caverns_FNB.x, h2_caverns_FNB.y))
    h2_caverns_FNB['Coordinates'] = h2_caverns_FNB['Coordinates'].apply(Point)
    print(h2_caverns_FNB)

    if options["FNB_hydrogen_underground_storage_locations"]:
                
        # For case 2: to define geodataframe of hydrogen bus
        # Select buses with carrier 'AC' and 'H2'
        df_AC = n.buses[n.buses.carrier == 'AC']
        df_H2 = n.buses[n.buses.carrier == 'H2']

        # Overwrite x, y coordinates from df_AC for df_H2
        df_H2.loc[:,'x'] = df_AC['x'].values
        df_H2.loc[:,'y'] = df_AC['y'].values

        # GeoDataFrame for H2 bus from PyPSA and FNB 
        n_H2 = gpd.GeoDataFrame(df_H2, geometry=df_H2.apply(lambda r: shapely.geometry.Point(r.x, r.y), axis=1), crs=4326).reset_index()
        FNB_H2 = gpd.GeoDataFrame(h2_caverns_FNB, geometry=h2_caverns_FNB["Coordinates"], crs=4326)
        
        #n_H2 = n_H2.to_crs(epsg=4326).reset_index()
        #FNB_H2 = FNB_H2.to_crs(epsg=4326)
        n_H2_DE = n_H2[n_H2['Bus'].str.contains('DE')] # H2 bus in Germany
        print(n_H2)
        print(FNB_H2)
        print(n_H2_DE)

        # Create NumPy arrays of geometries
        n_H2_DE_geometries = np.array(n_H2_DE['geometry'].astype(object).apply(lambda x: (x.x, x.y)).tolist())
        FNB_H2_geometries = np.array(FNB_H2['geometry'].astype(object).apply(lambda x: (x.x, x.y)).tolist())
        
        # Initialize an empty array for distances
        num_n_H2_DE = len(n_H2_DE_geometries)
        num_FNB_H2 = len(FNB_H2_geometries)
        # Create an empty array to store the distances
        distances = np.zeros((num_n_H2_DE, num_FNB_H2))
        """
        for i in range(num_n_H2_DE):
            for j in range(num_FNB_H2):
                distances = np.linalg.norm(i-j)
        print(distances)
        """
        for i in range(num_n_H2_DE):
            for j in range(num_FNB_H2):
                # Calculate the Euclidean distance
                distances[i][j] = np.sqrt(np.sum((n_H2_DE_geometries[i] - FNB_H2_geometries[j]) ** 2)) * 111.32 # 1 degree of latitude is approximately 111.32 km
                #distances = np.linalg.norm(i-j)
        print(distances)
        '''
        min_dists = np.min(distances, axis=0)
        row_indices = np.argmin(distances, axis=0)
        column_indices = np.arange(distances.shape[1])
        result = [(min_d, row_indices[i], column_indices[i]) for i, min_d in enumerate(min_dists)]
        '''
        """        
        min_values = np.min(distances, axis=0)
        used_indices = np.full(distances.shape[1], -1)
        row_indices = np.argmin(distances, axis=0)
        result = []
        for i, min_val in enumerate(min_values):
            col_indices = np.where(distances[:, i] == min_val)[0]
            for idx in col_indices:
                if used_indices[i] == -1 or idx > used_indices[i]:
                    used_indices[i] = idx
                    result.append((min_val, idx, i))
                    break
        ...
        for col in range(distances.shape[1]):
            column = distances[:, col]
            min_value = np.inf
            min_row = None

            for row in range(distances.shape[0]):
                if row not in used_indices:
                    if column[row] < min_value:
                        min_value = column[row]
                        min_row = row

            if min_row is not None:
                result.append((min_value, min_row, col))
                used_indices.add(min_row)

        print(result)
        
        df = pd.DataFrame(result, columns=['distance','H2_DE_idx','FNB_idx'])
        df['FNB'] = FNB_H2['name']
        df['geometry'] = FNB_H2['geometry']
        
        H2_DE = n_H2_DE.reset_index()
        del H2_DE['index']

        # mapping PyPSA bus for FNB
        df['PyPSA_bus'] = H2_DE.loc[df['H2_DE_idx'].values, 'Bus'].values
        h2_caverns_FNB['PyPSA_bus'] = df['PyPSA_bus'] 
        print(h2_caverns_FNB)
        # mapping to update H2 node with FNB location (question this!!)
        #mapping = df.set_index('PyPSA_bus')['geometry'].to_dict()
        #H2_DE['geometry'] = H2_DE['Bus'].map(mapping).combine_first(H2_DE['geometry'])
        #print(H2_DE)

        logger.info(
            "Remove all H2 storage in Germany"
        )
        stores = n.stores.reset_index()
        #h2_store_DE = stores[stores['Store'].str.startswith('DE') & (stores['Store'].str.endswith('H2 Store'))]
        h2_store_DE = stores[stores['Store'].str.startswith('DE') & (stores['carrier'] == 'H2')]
        print(h2_store_DE)
        h2_store_ids = h2_store_DE['Store'].tolist()
        filtered_stores = stores[~stores['Store'].isin(h2_store_ids)] 

        n.mremove("Store", h2_store_ids)


        logger.info("Add FNB hydrogen underground storage")

        #FNB_bus = spatial.h2[spatial.h2.nodes.isin(['DE0 39','DE0 21','DE0 11','DE0 33','DE0 6','DE0 1','DE0 19','DE0 13','DE0 26','DE0 25','DE0 14','DE0 36'])]
        FNB_bus = [node for node in spatial.h2.nodes if node in ['DE0 39', 'DE0 21', 'DE0 11', 'DE0 33', 'DE0 6', 'DE0 1', 'DE0 19', 'DE0 13', 'DE0 26', 'DE0 25', 'DE0 14', 'DE0 36']]
        #h2_capital_cost = costs.at["hydrogen storage underground", "fixed"] # set this if have cost info
        #if options["H2_network"]:
        #    FNB_spatial = h2_caverns_FNB['capacity'].rename(index=lambda x: str(x) + " FNB")
        #else:
        #    FNB_spatial = h2_caverns_FNB['capacity'].sum()
        # Create buses for the new hydrogen storage locations
        #n.madd("Bus", h2_caverns_FNB.PyPSA_bus + " H2", carrier="H2", location=nodes)
        #n.madd("Bus", nodes + " H2", location=nodes, carrier="H2", unit="MWh_LHV")
        n.madd("Bus", FNB_bus, location=FNB_bus, carrier="H2", unit="MWh_LHV")
        # add hydrogen storage units from FNB
        n.madd(
            "Store",
            #nodes + str(h2_caverns_FNB["name"]) + " H2 Store",
            #h2_caverns_FNB.PyPSA_bus.str[:-2].str.strip() + " " + h2_caverns_FNB.name + " H2 Store",
            FNB_bus + " " + h2_caverns_FNB.name + " H2 Store",
            #spatial.h2.FNB + " H2 Store",
            #bus= h2_caverns_FNB.PyPSA_bus,
            bus=FNB_bus,
            e_nom_extendable=False,
            #e_nom=FNB_spatial,
            e_nom=h2_caverns_FNB["capacity"].values,
            e_cyclic=True,
            carrier="H2 Store",
    ...
    # if have, FNB hydrogen stored overground (where not already underground)
    #h2_capital_cost = costs.at[
    #    "hydrogen storage tank type 1 including compressor", "fixed"
    #]
    #nodes_overground_FNB = h2_caverns_FNB.index.symmetric_difference(nodes)

#    n.madd(
#        "Store",
#        nodes_overground_FNB + " H2 Store",
#        bus=nodes_overground_FNB + " H2",
#        e_nom_extendable=False,
#        e_cyclic=True,
#        carrier="H2 Store",
#        #capital_cost=h2_capital_cost,
#    )
    ...
    # for the rest h2_caverns (##that not h2_caverns_DE:)
    cavern_types = snakemake.params.sector["hydrogen_underground_storage_locations"]
    h2_caverns = pd.read_csv(snakemake.input.h2_cavern, index_col=0)
    h2_store_DE = stores[stores['Store'].str.startswith('DE') & (stores['carrier'] == 'H2')]

#    if not h2_caverns[h2_caverns.apply(lambda row: 'DE H2' in row.values, axis=1)].empty:
