...
# For first case: no UG H2 storage at all
def remove_UG_hydrogen_storage(n, costs):

    logger.info("Remove UG hydrogen storage")
    allstore = n.stores.reset_index()
    allstore_UG = allstore[allstore['Store'].str.endswith('2030') & (allstore['carrier']=="H2 Store")]
    
    allstore_UG_ids = allstore_UG['Store'].tolist()
    n.mremove("Store", allstore_UG_ids)
...
if __name__ == "__main__":
    ...
    # For first case: Remove hydrogen storage components from PyPSA network
    remove_hydrogen_storage(n, costs)
