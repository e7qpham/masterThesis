...
def add_h2_elec_cosntraint(n):
    """
    Add constraint for H2 Electrolysis capacity for Germany.
    """
    #Create a DataFrame with country, carrier and max_capacity
    data = {
    'country': ['DE'],
    ...
    global_max = 40000
    agg_p_nom_max = pd.DataFrame(data)
    agg_p_nom_max.set_index('country', inplace=True)
    logger.info("Adding H2 generation capacity constraints in Germany") # add to log report
    # Select Links p_nom from n.model variables
    p_nom = n.model["Link-p_nom"]
    # Create a list with the electrolyzers names for all buses
    carriers_to_keep = ["H2 Electrolysis", "SMR CC"]
    mask = n.links['carrier'].isin(carriers_to_keep)
    links = n.links[mask].rename_axis(index="Link-ext")
    links['bus1'] = links['bus1'].str.replace(' H2', '')
    links_index = links.index
    links_ext = p_nom.loc[links_index]
    grouper = links.bus1.map(n.buses.country)
    lhs = links_ext.groupby(grouper).sum().rename(bus1="country")
    maximum = xr.DataArray(agg_p_nom_max["max"].dropna())
    index = maximum.indexes["country"].intersection(lhs.indexes["country"])
    if not index.empty:
        n.model.add_constraints(
        lhs.sel(country=index) <= maximum.loc[index], name="agg_link_p_nom_max"
        )
        electrolyzer_ext = n.links.query("carrier == 'H2 Electrolysis'").index
        electrolyzer = p_nom.loc[electrolyzer_ext]
        lhs = electrolyzer.sum()
    #if not global_max.empty:
    n.model.add_constraints(
    lhs <= global_max, name="global_electrolyzer_max"
    )
